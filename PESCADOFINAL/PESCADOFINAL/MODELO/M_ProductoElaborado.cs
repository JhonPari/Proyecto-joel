	...
	public class M_ProductoElaborado
	{
		...
		public int existeModelo(string nombre)
		{
			...
			try
			{
				...
				cmd.Parameters.Add(
					new SqlParameter("@nombre", SqlDbType.VarChar, 80)
					{
						Value = nombre
					});
			...
			decimal cantidad)
		{
			...
			{
				SqlCommand cmd =
					new SqlCommand("PELABORADO_REGISTRAR", c);
				...
				cmd.Parameters.Add(
					new SqlParameter("@descripcion", SqlDbType.VarChar, 120)
					{
						Value = (object)descripcion ?? DBNull.Value
					});

				cmd.Parameters.Add(
					new SqlParameter("@precioVenta", SqlDbType.Decimal)
					{
						Value = precioVenta
					});
			...
			decimal cantidad)
		{
			...
			{
				SqlCommand cmd =
					new SqlCommand("PELABORADO_EDITAR", c);
		...
		public int eliminarModelo(int codigo)
		{
			...
			{
				SqlCommand cmd =
					new SqlCommand("PELABORADO_ELIMINAR", c);
		...
		public int aumentarStockModelo(int codigo, decimal cantidad)
		{
			...
			{
				SqlCommand cmd =
					new SqlCommand("PELABORADO_AUMENTAR_STOCK", c);
		...
		public DataTable listarModelo()
		{
			...
			{
				SqlCommand cmd =
					new SqlCommand("PELABORADO_LISTAR", c);
